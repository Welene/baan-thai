...
exports.handler = async (event) => {
  try {
    ...
    if (!email || !password || !name || !username) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Email, lösenord, namn och användarnamn krävs' })
      };
    }
    ...
    let existingUser;
    try {
      existingUser = await dynamodb.send(new QueryCommand(checkEmailParams));
    } catch (err) {
    ...
    if (existingUser.Items && existingUser.Items.length > 0) {
      return {
        statusCode: 409,
        body: JSON.stringify({ error: 'Email finns redan registrerad' })
      };
    }
