...
var { DynamoDBDocumentClient, ScanCommand } = require("@aws-sdk/lib-dynamodb");
var client = new DynamoDBClient({ region: "eu-north-1" });
var dynamoDb = DynamoDBDocumentClient.from(client);
var TABLE_NAME = process.env.RESTAURANT_TABLE || "RestaurantTable";
  ...
  try {
    ...
    const items = result.Items.map((item) => {
      ...
      return {
        ...
        name: item.name,
        title: item.name,
        // Både name och title för kompatibilitet
    ...
    return {
      ...
      body: JSON.stringify({
        error: "Kunde inte h\xE4mta meny",
