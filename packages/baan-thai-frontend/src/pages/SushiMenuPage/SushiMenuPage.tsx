  ...
  useEffect(() => {
    ...
    fetch("https://6kpqtftjk5.execute-api.eu-north-1.amazonaws.com/api/menu", {
      ...
      .then(data => {
        ...
        console.log('Produkt-ID:n som hämtades:', unique.map((it: any) => it.id).sort((a, b) => a - b));
        console.table(unique.map((it: any) => ({ 
          id: it.id, 
          namn: it.name, 
  ...
  return (
    <>
      ...
      {!loading && sortedCategories.map((categoryKey) => (
        <section key={categoryKey} id={`category-${categoryKey}`} className="category-section">
          <h2 className="category-title">{categoryNames[categoryKey] || categoryKey}</h2>
          <div className="grid">
            {groupedItems[categoryKey].map((item: any) => (
              <MenuCard key={item.id} menuItem={item} onAddToCart={handleAddToCart} />
            ))}
          </div>
        </section>
      ))}
